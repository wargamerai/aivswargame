...
class NomonhanEnvPerfect:
    ...
    def get_state_key(self, side, units, bridge_destroyed):
        # 【修正箇所】今のターンが「JPかSOVか」を状態の先頭にくっつけることでエラーを完全に防止！
        alive_units = sorted([(u['id'], u['r'], u['c'], u['steps']) for u in units if u['steps'] > 0])
        return f"{side}_{str(alive_units)}_B{1 if bridge_destroyed else 0}"
        ...
        for episode in range(1, episodes + 1):
            ...
            while game_active and turn <= 8:
                for side in ["JP", "SOV"]:
                    ...
                    # 状態取得時に side を渡す
                    state = self.get_state_key(side, units, bridge_destroyed)
                    ...
                    if game_active:
                        # ここでも side を渡す
                        next_state = self.get_state_key(side, units, bridge_destroyed)
